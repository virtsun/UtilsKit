...
#define BACKGROUND_SIZE 275.f
#define PATH_SIZE 200.f
#define IMAGE_SIZE 45.f
#define TOUCH_SIZE 70.f
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        _sharers = [@[[CFSharer pinterest],
                ...
                [CFSharer dropbox],
                [CFSharer mail],
                [CFSharer photoLibrary]] mutableCopy];
...
- (void)layoutSubviews {
    ...
    if(_circleIsVisible) {
        ...
        _backgroundLayer.position = CGPointMake(self.bounds.size.width + BACKGROUND_SIZE/2.0f, CGRectGetMidY(self.bounds));
    }
...
- (void)setUpCircleLayers {  
    ...
    _backgroundLayer.frame = CGRectMake(CGRectGetMidX(self.bounds) - BACKGROUND_SIZE/2.0f, CGRectGetMidY(self.bounds) - BACKGROUND_SIZE/2.0f, BACKGROUND_SIZE, BACKGROUND_SIZE);
    _backgroundLayer.position = CGPointMake(self.bounds.size.width + BACKGROUND_SIZE/2.0f, CGRectGetMidY(self.bounds));
    _backgroundLayer.fillColor = [UIColorFromRGBA(0xffffff, .8f) CGColor];
    ...
    for(int i = 0; i < _numberSharersInCircle; i++) {
        ...
        if(i == 5 && _sharers.count > 6)
            sharer = [[CFSharer alloc] initWithName:@"More" imageName:@"images.bundle/more.png"];
        else
            sharer = _sharers[i];
        ...
        imageLayer.position = CGPointMake(BACKGROUND_SIZE/2.0f, BACKGROUND_SIZE/2.0f);
        imageLayer.contents = (id)image.CGImage;
        imageLayer.shadowColor = [UIColor colorWithRed:213.0f/255.0f green:213.0f/255.0f blue:213.0f/255.0f alpha:1.0].CGColor;
        ...
        [_backgroundLayer addSublayer:_imageLayers[i]];
    }
...
- (void)setUpSharingOptionsView {
    ...
    UINavigationBar *_topBar = [[UINavigationBar alloc] init];
    _topBar.frame = CGRectMake(0, 0, CGRectGetWidth(self.bounds), 64);
    _topBar.translucent = NO;

    UINavigationItem*_topItem = [[UINavigationItem alloc] init];
    _topBar.items = @[_topItem];
    _topItem.title = @"Sharing Options";

    [_sharingOptionsView addSubview:_topBar];

    
    // Add table view.
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0.0f,
            CGRectGetHeight(_topBar.frame) + CGRectGetMinY(_topBar.frame),
            _sharingOptionsView.frame.size.width,
            _sharingOptionsView.frame.size.height)];
    tableView.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
    tableView.delegate = self;
    tableView.dataSource = self;
    tableView.separatorStyle = UITableViewCellSeparatorStyleSingleLine;
    tableView.separatorColor = UIColorFromRGBA(0x000000, .1f);
    tableView.rowHeight = 60.0f;
    [_sharingOptionsView addSubview:tableView];
    
    // Add the close button.
    UIButton *closeButton = [UIButton buttonWithType:UIButtonTypeCustom];
    closeButton.frame = CGRectMake(0,0.0f,60.0f,45.0f);
    [closeButton setTitle:@"Close" forState:UIControlStateNormal];
    [closeButton setTitleColor:UIColorFromRGB(0x000000) forState:UIControlStateNormal];
    [closeButton setTitleColor:UIColorFromRGBA(0x000000,0.6f) forState:UIControlStateHighlighted];
    ...
    _topItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:closeButton];
...
- (CGPoint)touchLocationAtPoint:(CGPoint)point {
    ...
    // See if the new point is outside of the circle's radius.
        ...
        point.x = _origin.x - (BACKGROUND_SIZE/2.0f - TOUCH_SIZE/2.0f) * cos(angle);
        point.y = _origin.y + (BACKGROUND_SIZE/2.0f - TOUCH_SIZE/2.0f) * sin(angle);
    }
...
- (void)hide {
    ...
    // Construct the animation.
    animation.fromValue = [_backgroundLayer valueForKey:@"position"];
    animation.toValue = [NSValue valueWithCGPoint:CGPointMake(self.bounds.size.width + BACKGROUND_SIZE/2.0f, CGRectGetMidY(self.bounds))];
