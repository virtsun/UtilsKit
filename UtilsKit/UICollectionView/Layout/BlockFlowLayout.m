...
#import "BlockFlowLayout.h"

@interface BlockFlowLayout ()
// todo put these back
...
@implementation BlockFlowLayout
...
- (void) initialize {
    ...
    self.indexPathByPosition = [@{} mutableCopy];
}
...
- (void) prepareLayout {
    ...
    if (isVert)
        unrestrictedRow = (int) ((self.collectionView.frame.size.height / [self blockPixels].height)+1);
    else
        unrestrictedRow = (int) ((self.collectionView.frame.size.width / [self blockPixels].width)+1);
...
- (BOOL) traverseOpenTiles:(BOOL(^)(CGPoint))block {
    ...
    // the double ;; is deliberate, the unrestricted dimension should iterate indefinitely
    for(int unrestrictedDimension = (int)(isVert? self.firstOpenSpace.y : self.firstOpenSpace.x);; unrestrictedDimension++) {
        for(int restrictedDimension = 0; restrictedDimension<[self restrictedDimensionBlockSize]; restrictedDimension++) {
...
- (void) clearPositions {
    self.indexPathByPosition = [@{} mutableCopy];
    self.positionByIndexPath = [@{} mutableCopy];
}
...
- (NSIndexPath*)indexPathForPosition:(CGPoint)point {
    ...
    if (self.indexPathByPosition.count ==0)
        return nil;
...
- (float) restrictedDimensionBlockSize {
    BOOL isVert = self.direction == UICollectionViewScrollDirectionVertical;

    float size = isVert? self.collectionView.frame.size.width / self.blockPixels.width : self.collectionView.frame.size.height / self.blockPixels.height;
